...
// WaterDevice 基于water库的Linux TUN设备
...
// NewWaterDevice 创建基于water的TUN设备
...
func (w *WaterDevice) Start() error {
	...
	// 设置IP地址
	if w.config.Address != "" {
		// 使用系统命令配置IP地址（需要root权限）
		// 这里简化处理，实际需要调用netlink或ip命令
		// 暂时跳过，由用户手动配置
	}
...
func (w *WaterDevice) FD() (uintptr, error) {
	if w.ifce == nil {
		return 0, fmt.Errorf("设备未初始化")
	}
	// water库的新版本已移除File()方法，暂时不支持获取FD
	return 0, fmt.Errorf("FD获取暂不支持，请更新water库或使用其他方法")
}
